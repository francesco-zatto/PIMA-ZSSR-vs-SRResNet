...
class ConvBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, normalize=True, activation=True):
        ...
        self.batch_norm = nn.BatchNorm2d(out_channels) if normalize else nn.Identity()
        self.prelu = nn.PReLU(num_parameters=out_channels) if activation else nn.Identity()
...
class ResidualBlock(torch.nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, normalize=True):
        super().__init__()
        self.conv1 = ConvBlock(in_channels=in_channels, out_channels=out_channels, kernel_size=kernel_size, normalize=normalize)
        self.conv2 = ConvBlock(in_channels=out_channels, out_channels=out_channels, kernel_size=kernel_size, normalize=normalize, activation=False)
...
class SRResNet(nn.Module):
    ...
    """
    def __init__(self, num_channels: int = 64, num_blocks: int = 16, upscale_factor: int = 4, use_batch_norm : bool = True, final_activation : bool = True):
        super().__init__()
        ...
        self.residual_blocks = nn.Sequential(*[ResidualBlock(in_channels=num_channels, out_channels=num_channels, normalize=self.use_batch_norm) for _ in range(num_blocks)])
        self.secondary_conv = ConvBlock(in_channels=num_channels, out_channels=num_channels, kernel_size=3, normalize=self.use_batch_norm, activation=False)
